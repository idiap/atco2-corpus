...
"""\
Script that fine-tune a Wav2Vec 2.0 model from HuggingFace. 
1) we download a pre-trained model,
2) We fine-tune it on the passed dataset. 
...
class ModelArguments:
    ...
    vocab_path: Optional[str] = field(
        default=None,
        metadata={"help": "Path to vocabulary file, or path to local folder"},
    )
...
class DataTrainingArguments:
    ...
    eval_dataset_name: str = field(
        metadata={
            "help": "The configuration name of the dataset to use for evaluation (via the datasets library)."
        }
    )
...
def main():
    ...
    # 2. remove some special characters from the datasets (DONE: directly in the data loader)
    ...
    if tokenizer_name_or_path is None:
        ...
        if model_args.vocab_path is not None:
            vocab_general_path = os.path.join(model_args.vocab_path)
            # create directory for tokenizer
            if not os.path.isfile(vocab_file):
                os.makedirs(tokenizer_name_or_path, exist_ok=True)

            shutil.copyfile(model_args.vocab_path, vocab_file)
            print("Vocabulary file copied successfully.")

        # no vocab file provided, thus, create it
        else:
    ...
    model = AutoModelForCTC.from_pretrained(
        ...
        config=config,
        use_auth_token=data_args.use_auth_token,
        ignore_mismatched_sizes=True,  # This is new to avoid the mismatch at the last head (output head or 'lm-head')
    )
